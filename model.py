...
from utils import generate_aug_daj_mat, generate_drop_daj_mat, graph_aug_rank_nodes, graph_drop_rank_nodes, generate_drop_daj_mat2, generate_drop_daj_mat3, generate_drop_daj_mat_ran_min
import random
import gc
...
class DOSE_aug(BasicModel):
    ...
    def cal_cos_sim(self):
        ...
        # cos_mat = torch.from_numpy(cos_mat.astype(np.float32)).clone()
...
class DOSE_aug2(BasicModel):
    ...
    def cal_cos_sim(self):
        ...
        cos_mat = cosine_similarity(all_user_r, all_item_r)
...
class DOSE_drop2(BasicModel):
    ...
    def load(self, path):
        ...
        self.feat_mat, _, _, self.row_sum = self.generate_feat(self.config['dataset'], is_updating=True)
        self.update_feat_mat()
class DOSE_drop_test(BasicModel):
    def __init__(self, model_config):
        # ランダムに削除
        super(DOSE_drop_test, self).__init__(model_config)
        self.embedding_size = model_config['embedding_size']
        self.n_layers = model_config['n_layers']
        self.dropout = model_config['dropout']
        self.feature_ratio = model_config['feature_ratio']  # Template%
    ...
    def cal_cos_sim_v2(self):
        ...
        sample_n_user = 10000
        sample_n_items = 10000

        sample_user = torch.tensor([random.randint(0, self.n_users - 1) for i in range(sample_n_user)])
        sample_item = torch.tensor([random.randint(0, self.n_items - 1) for i in range(sample_n_items)])

        sample_user.sort()
        sample_item.sort()

        user_index_mapping = {index: user for index, user in enumerate(sample_user)}
        item_index_mapping = {index: item for index, item in enumerate(sample_item)}

        # Extract rows corresponding to sampled users and items
        sampled_users_r = all_users_r[sample_user, :]
        sampled_items_r = all_items_r[sample_item, :]

        sampled_users_r = sampled_users_r.to('cpu').detach().numpy().copy()
        sampled_items_r = sampled_items_r.to('cpu').detach().numpy().copy()

        x = cosine_similarity(sampled_users_r, sampled_items_r)
        ...
        # Adjust indices to match the original indexing of all items
        adjusted_idx = [
            [int(torch.div(idx[0][i], sample_n_items, rounding_mode='floor')), (int(torch.fmod(idx[0][i], sample_n_items)))]
            for i in range(self.aug_num)]

        # Map adjusted indices back to original indices
        # ユーザとアイテムのインデックスを元のUserIDとItemIDに変換しながら、original_idxも作成
        original_idx = [
            [user_index_mapping[adjusted_idx[i][0]], item_index_mapping[adjusted_idx[i][1]]]
            for i in range(self.aug_num)
        ]
        del cos_mat, user_index_mapping, item_index_mapping
        torch.cuda.empty_cache()

        return original_idx  # return list [user_id, item_id]
...
class DOSE_drop4(BasicModel):
    def __init__(self, model_config):
        # RD + Min
        super(DOSE_drop4, self).__init__(model_config)
        self.embedding_size = model_config['embedding_size']
        self.n_layers = model_config['n_layers']
        self.dropout = model_config['dropout']
        self.feature_ratio = model_config['feature_ratio']  # Template%
    ...
    def generate_drop_graph(self, dataset):
        # new graph after adding some interactions
        aug_idx = self.cal_cos_sim()
        aug_adj_mat = generate_drop_daj_mat_ran_min(dataset, aug_idx, aug_rate=0.9)
        degree = np.array(np.sum(aug_adj_mat, axis=1)).squeeze()
        degree = np.maximum(1., degree)
        d_inv = np.power(degree, -0.5)  # 累乗
        d_mat = sp.diags(d_inv, format='csr', dtype=np.float32)
...
class DOSE_test(BasicModel):
    ...
    def load(self, path):
        ...
        self.feat_mat, _, _, self.row_sum = self.generate_feat(self.config['dataset'], is_updating=True)
        self.update_feat_mat()
class DOSE_drop_drop(BasicModel):
    def __init__(self, model_config):
        super(DOSE_drop_drop, self).__init__(model_config)
        self.embedding_size = model_config['embedding_size']
        self.n_layers = model_config['n_layers']
        self.dropout = model_config['dropout']
        self.feature_ratio = model_config['feature_ratio']  # Template%
        ...
        normal_(self.embedding.weight, std=0.1)
        self.to(device=self.device)
        self.norm_aug_adj = self.generate_drop_graph2(model_config['dataset'])
        self.norm_drop_adj = self.generate_drop_graph(model_config['dataset'])
        self.times = model_config.get('times', 0.1)
        # self.aug_num = (self.times) * len(self.feat_mat)
    ...
    def generate_drop_graph(self, dataset):
        ...
        norm_aug_adj = get_sparse_tensor(norm_aug_adj, self.device)
        return norm_aug_adj
    def generate_drop_graph2(self, dataset):
        # new graph after adding some interactions
        aug_adj_mat = generate_drop_daj_mat(dataset, aug_rate=self.aug_rate)
        degree = np.array(np.sum(aug_adj_mat, axis=1)).squeeze()
        degree = np.maximum(1., degree)
    ...
    def cal_cos_simv2(self):
        ...
        aug_idx2 = [
            ...
            in range(self.aug_num // 2)]
        aug_idx1.extend(aug_idx2)
        del aug_idx2, idx1, idx2, cos_mat2
        torch.cuda.empty_cache()
...
class DOSE_IMF(IGCN):
    def __init__(self, model_config):
        super(DOSE_IMF, self).__init__(model_config)
...
class DOSE_MultiVAE(BasicModel):
    def __init__(self, model_config):
        super(DOSE_MultiVAE, self).__init__(model_config)
        self.dropout = model_config['dropout']
        self.aug_rate = model_config['aug_rate']
        self.normalized_data_mat = self.get_data_mat(model_config['dataset'])
        self.normalized_data_mat2 = self.get_data_mat2(model_config['dataset'])
    ...
    def get_data_mat2(self, dataset):
        train_array = dataset.train_array
        train_array = random.sample(train_array, int(len(train_array) * self.aug_rate))
        train_array = np.array(train_array)
        data_mat = sp.coo_matrix((np.ones((len(train_array),)), np.array(train_array).T),
                                 shape=(self.n_users, self.n_items), dtype=np.float32).tocsr()
    ...
    def update_aug_adj(self):
        del self.normalized_data_mat, self.normalized_data_mat2
        torch.cuda.empty_cache()
        # gc.collect()
        self.normalized_data_mat = self.get_data_mat(self.config['dataset'])
        self.normalized_data_mat2 = self.get_data_mat2(self.config['dataset'])
    ...
    def ml_forward(self, users):
        ...
        profiles2 = self.normalized_data_mat[users, :]
        representations2 = get_sparse_tensor(profiles2, self.device)

        representations = NGCF.dropout_sp_mat(self, representations)
        representations = torch.sparse.mm(representations, self.encoder_layers[0].weight.t())
        representations += self.encoder_layers[0].bias[None, :]

        representations2 = NGCF.dropout_sp_mat(self, representations2)
        representations2 = torch.sparse.mm(representations2, self.encoder_layers[0].weight.t())
        representations2 += self.encoder_layers[0].bias[None, :]

        l2_norm_sq = torch.norm(self.encoder_layers[0].weight, p=2)[None] ** 2
        for layer in self.encoder_layers[1:]:
            representations = layer(torch.tanh(representations))
            representations2 = layer(torch.tanh(representations2))
            l2_norm_sq += torch.norm(layer.weight, p=2)[None] ** 2
        ...
        for layer in self.decoder_layers[:-1]:
            representations = torch.tanh(layer(representations))
            representations2 = torch.tanh(representations2)
            l2_norm_sq += torch.norm(layer.weight, p=2)[None] ** 2

        ssl_loss = self.cal_loss(representations, representations2)
        scores = self.decoder_layers[-1](representations)
        l2_norm_sq += torch.norm(self.decoder_layers[-1].weight, p=2)[None] ** 2
        return scores, kl, l2_norm_sq, ssl_loss

    def predict(self, users):
        scores, _, _, _ = self.ml_forward(users)
        if scores.shape[1] < self.n_items:
            padding = torch.full([scores.shape[0], self.n_items - scores.shape[1]], -np.inf, device=self.device)
            scores = torch.cat([scores, padding], dim=1)
        return scores
